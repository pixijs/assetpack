...
| srcSortOptions   | `object`, `(assetsSrc: PixiManifestEntry['src']) => PixiManifestEntry['src']` | If an object:<br /> • `order`: 'ascending' or 'descending' (default: 'descending')<br /> • `collatorOptions`: [Intl.Collator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Collator/Collator#options) options for natural or locale-aware sorting.<br /><br />If a function:<br /> a custom comparator for advanced sorting logic. <br /><br />Defaults to `{ order: 'descending' }`. |
...
When using the [TexturePacker plugin](/docs/guide/pipes/texture-packer), you should use the `texturePackerManifestMod` plugin to augment the manifest file. This ensures that the correct file size is used for the texture atlas files.
